...
func main() {
	...
	wordList := wordlebot.GetWordList("/Users/ben/Documents/WordleBot/wordle_words.txt")
	allPatterns := wordlebot.CreatePatterns()

	for len(wordList) > 2 {
		guessIndex := wordlebot.MakeGuess(wordList, allPatterns)
		...
		for i, c := range strings.Split(feedback, "") {
		...
		wordList = wordlebot.PruneWordList(responsePattern, wordList, guessIndex)
	}
